...
2. **Integrated AI Chatbot**: In Phases III, IV, and V, you must implement a conversational interface using OpenAI Chatkit, OpenAI Agents SDK, and Official MCP SDK. The bot must be able to manage the user's Todo list via natural language (e.g., "Reschedule my morning meetings to 2 PM").

3. **Cloud Native Deployment**: In Phases IV and V, you must deploy the chatbot locally on Minikube, and on the cloud on DigitalOcean Kubernetes (DOKS).
...
## Advanced Level Functionality on DigitalOcean (DOKS), Azure (AKS), or Google Cloud (GKE)
...
- Deploy to Azure (AKS)/Google Cloud (GKE)/DigitalOcean (DOKS)
- Deploy Dapr on GKE/AKS/DOKS use Full Dapr: Pub/Sub, State, Bindings (cron), Secrets, Service Invocation
